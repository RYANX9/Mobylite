...
export function isNewRelease(phone: Phone): boolean {
  ...
  if (!phone.release_year || !phone.release_month || !phone.release_day) {
    return false;
  }
  
  // Build a proper Date object from year/month/day
  // Note: JavaScript months are 0-indexed (0 = January, 11 = December)
  const releaseDate = new Date(
    phone.release_year,
    phone.release_month - 1,  // Convert to 0-indexed
    phone.release_day
  );
  ...
  // Return true if release date is within the last 30 days
