...
const ReviewForm: React.FC<ReviewFormProps> = ({ phoneId, onSuccess }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="rounded-2xl p-6 border mb-8" style={{ backgroundColor: color.borderLight, borderColor: color.borderLight }}>
      <div className="flex items-center justify-between mb-6">
        <h4 className="font-bold text-lg" style={{ color: color.text }}>Write Your Review</h4>
        <div className="flex items-center gap-3">
          <StarRating value={rating} onChange={setRating} variant="desktop" />
          <span className="text-2xl font-bold" style={{ color: color.text, fontFamily: font.numeric }}>
            {rating.toFixed(1)}
          </span>
        </div>
      </div>
      ...
      <div className="space-y-4">
        ...
        <ButtonPressFeedback
          type="submit"
          disabled={loading}
          className="w-full py-3 rounded-xl font-bold transition-all text-base"
          style={{ 
...
const ExpandableReviewCard: React.FC<ExpandableReviewCardProps> = ({
  ...
  const [hasVoted, setHasVoted] = useState(helpfulVotes.has(review.id));
  ...
  const estimatedLines = Math.ceil(review.body.length / 80); // ~80 chars per line
  ...
  const handleHelpful = async () => {
    ...
    if (onHelpful && !hasVoted && !isOwnReview) {
  ...
  // Get username - NEVER show "Anonymous"
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between pt-3 border-t" style={{ borderColor: color.borderLight }}>
        {needsExpansion && (
          <button
        ...
        {!isOwnReview && onHelpful && (
          <ButtonPressFeedback
            ...
            style={{ 
              color: hasVoted ? color.success : color.textMuted,
              backgroundColor: hasVoted ? color.successBg : 'transparent',
              border: `1px solid ${hasVoted ? color.success : color.border}`
            }}
            hoverStyle={!hasVoted ? { backgroundColor: color.borderLight } : {}}
          >
      ...
      if (data && data.reviews) {
        ...
        if (isAuthenticated()) {
          try {
            ...
            const userId = meResponse?.id || meResponse?.user?.id;
            
            if (userId) {
              const userRev = allReviews.find((r: Review) => r.user_id === userId);
              setUserReview(userRev || null);
              
              // Remove user's review from main list
              allReviews = allReviews.filter((r: Review) => r.user_id !== userId);
            }
          } catch (err) {
        ...
        // Sort by helpful_count DESC and take top 3
      ...
      {isAuthenticated() ? (
        <>
          {userReview ? (
            ...
            // ✅ User hasn't reviewed - show form or button
            <>
              {showForm ? (
                <ReviewForm 
                  phoneId={phoneId} 
