...
export default function DesktopCompare({ phones, setComparePhones, setView, setSelectedPhone }: DesktopCompareProps) {
  ...
  useEffect(() => {
    const loadFromHash = async () => {
      ...
      if (hash.startsWith('#compare=') && window.location.pathname === '/' && phones.length === 0) {
        const urlPart = hash.replace('#compare=', '');
        
        if (urlPart.includes('-vs-')) {
          const phoneNames = urlPart.split('-vs-');
          await loadPhonesFromNames(phoneNames);
        } else {
          const ids = urlPart.split(',').map(id => parseInt(id.trim()));
          if (ids.length > 0) {
            await loadPhonesFromURL(ids);
          }
        }
      }
    };
  ...
  useEffect(() => {
    if (phones.length > 0 && window.location.pathname === '/') {
      ...
      window.history.replaceState(null, '', `#compare=${phoneNames}`);
    }
  }, [phones]);
      ...
      if (isAuthenticated()) {
        fetch(`${API_BASE_URL}/comparisons`, {
          ...
          body: JSON.stringify({ 
            ...
            comparisonName: `${phones.map(p => p.model_name).join(' vs ')} vs ${phone.model_name}`
          })
        }).catch(console.error);
      }
